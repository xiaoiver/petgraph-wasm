...
impl DiGraph {
    ...
    pub fn neighbors_directed(
        ...
        let neighbor_vec: Vec<usize> = self
            .graph
            .neighbors_directed(graph::NodeIndex::new(index), direction.try_into()?)
            .map(|neighbor: graph::NodeIndex| neighbor.index())
            .collect();
        Ok(neighbor_vec)
    }
...
mod tests {
    ...
    #[wasm_bindgen_test]
    fn can_get_neighbors_by_index() {
        let (mut g, _nodes, _edges) = new_test_graph();
        let neighbors = g.neighbors(0).unwrap();

        assert_eq!(neighbors, vec![2, 3]);
    }

    #[wasm_bindgen_test]
    fn can_get_neighbor_directed_by_index() {
        let (mut g, _nodes, _edges) = new_test_graph();
        let neighbors = g.neighbors_directed(0, Direction::Outgoing).unwrap();
        assert_eq!(neighbors, vec![2, 3]);

        let neighbors = g.neighbors_directed(0, Direction::Incoming).unwrap();
        assert_eq!(neighbors, vec![4, 1]);
    }

    #[wasm_bindgen_test]
    fn can_get_neighbor_undirected_by_index() {
        let (mut g, _nodes, _edges) = new_test_graph();
        let neighbors = g.neighbors_undirected(0).unwrap();
        assert_eq!(neighbors, vec![2, 3, 4, 1]);
    }
}